...
namespace packet_exchange
{
    ...
    void send_item(CUser* user, CUser* exchangeUser, Packet buffer, bool pvp)
    {
        ...
        packet.destSlot = util::deserialize<std::uint8_t>(buffer, 5, 6);

        auto bag = util::deserialize<std::uint8_t>(buffer, 2, 6);
        auto slot = util::deserialize<std::uint8_t>(buffer, 3, 6);
        ...
        packet.count = util::deserialize<std::uint8_t>(buffer, 4, 6);
