...
int CTexture::Create(CTexture* texture, const char* path, const char* fileName/*.tga*/, int w, int h)
{
    typedef int(__thiscall* LPFN)(CTexture*, const char*, const char*, int, int);
    return (*(LPFN)0x57B560)(texture, path, fileName, w, h);
}

void CTexture::Draw(CTexture* texture, D3DCOLOR color, int x, int y)
{
    typedef void(__thiscall* LPFN)(CTexture*, D3DCOLOR, int, int);
    (*(LPFN)0x57BD90)(texture, color, x, y);
}

void CTexture::Draw(CTexture* texture, int x, int y, float z)
{
    typedef void(__thiscall* LPFN)(CTexture*, int, int, float);
    (*(LPFN)0x57B680)(texture, x, y, z);
}

void CTexture::Draw(CTexture* texture, D3DCOLOR diffuse, 
    int x, int y, float z, 
    int w, int h, 
